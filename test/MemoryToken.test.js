...
contract("Memory Tokens Blockchain Game | Ignacio Ceaglio", (accounts) => {
  let token;
  ...
  describe("deployment", async () => {
    ...
    it("has a name", async () => {
      ...
      assert.equal(name, "Memory Tokens Blockchain Game | Ignacio Ceaglio");
    });

    it("has a symbol", async () => {
      const symbol = await token.symbol();
      assert.equal(symbol, "MTBGIC");
    });
  });
